    ...
    for action_i in range(total_actions):
        ...
        for action_timestep in range(action_repeat):
            ...
            if collision:
                ...
                break
        p = sensor_readings.tolist()
        network_params = network_params+[p+[action]+[collision]]
    # new_array=[]
    # for i in range(100):
    #     num = random.randint(0,len(network_params)-1)
    #     new_array.append(network_params[num])
    # a = np.array(new_array)
    a = np.array(network_params)
